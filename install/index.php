...
class perfcode_blankd7simple extends CModule
{
    var $exclusionAdminFiles;

    function __construct()
    {
        $this->MODULE_ID = 'perfcode.blankd7simple';
        $this->MODULE_NAME = Loc::getMessage('PERFCODE_BLANKD7SIMPLE_MODULE_NAME');
        $this->MODULE_DESCRIPTION = Loc::getMessage('PERFCODE_BLANKD7SIMPLE_MODULE_DESCRIPTION');
    ...
    function DoInstall()
    {
        ...
        if (!ModuleManager::isModuleInstalled('iblock')) {
            $errors = Loc::getMessage('PERFCODE_BLANKD7SIMPLE_MODULE_NOT_INSTALLED_IBLOCK');
        } elseif (!ModuleManager::isModuleInstalled('sale')) {
            $errors = Loc::getMessage('PERFCODE_BLANKD7SIMPLE_MODULE_NOT_INSTALLED_SALE');
        } elseif (!ModuleManager::isModuleInstalled('catalog')) {
            $errors = Loc::getMessage('PERFCODE_BLANKD7SIMPLE_MODULE_NOT_INSTALLED_CATALOG');
        } elseif (!ModuleManager::isModuleInstalled('currency')) {
            $errors = Loc::getMessage('PERFCODE_BLANKD7SIMPLE_MODULE_NOT_INSTALLED_CURRENCY');
        } else {
    ...
    private function createDirectories($documentRoot)
    {
        ...
        $blankd7simpleDirectoryPath = "{$perfcodeDirectoryPath}/blankd7simple";
        if (!Directory::isDirectoryExists($blankd7simpleDirectoryPath)) {
            Directory::createDirectory($blankd7simpleDirectoryPath);
        }
    }
    ...
    private function deleteDirectories()
    {
        ...
        DeleteDirFilesEx("/{$uploadDirectoryPath}/perfcode/blankd7simple");
    }
    ...
    function RegisterEvents()
    {
        EventManager::getInstance()->registerEventHandler(
            ...
            'OnEpilog',
            $this->MODULE_ID,
            'Perfcode\Blankd7simple\Events\MainEvents',
            'EpilogHandler',
            1000
        );
    }
